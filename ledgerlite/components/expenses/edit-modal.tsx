...
interface EditExpenseModalProps {
  ...
  isOpen: boolean;
  onClose: () => void;
  onSuccess: (updatedExpense: ExpenseItem) => void;
}

/**
 * EditExpenseModal Component
 * 
 * Production-style Form Component.
 * Contains and validates its own local form states, manages its own PUT request execution,
 * and notifies the parent to update the list only after a successful API save.
 * 
 * Core Design Patterns:
 * 1. State Encapsulation: Sub-state hooks (description, category, amount, error states)
 *    are kept inside this modal, freeing up parent memory and reducing unnecessary parent re-renders.
 * 2. Unidirectional Callback: Runs `onSuccess(updatedRecord)` upon completing database updates,
 *    letting the parent apply local optimistic changes.
 */
export default function EditExpenseModal({
  ...
  async function handleUpdateExpense(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const res = await fetch(`/api/routes/expenses/${expense.id}`, {
        method: "PUT",
        headers: {
      ...
      // Notify parent to save changes locally
      onSuccess({
        ...expense,
        ...updatedData,
      });
